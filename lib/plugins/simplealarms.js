...
function init() {
  ...
  simplealarms.compareBGToTresholds = function compareBGToTresholds(scaledSGV, sbx) {
    ...
    if (scaledSGV > sbx.scaleMgdl(sbx.settings.thresholds.bgHigh)) {
      ...
      result.title = levels.toDisplay(levels.URGENT) + ' MAGAS'; // was ' HIGH' - needs to be changed to translate()
      ...
      result.title = levels.toDisplay(levels.WARN) + ' MAGAS'; // was ' HIGH' - needs to be changed to translate()
