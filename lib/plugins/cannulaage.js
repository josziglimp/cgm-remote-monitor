...
function init(ctx) {
  ...
  cage.findLatestTimeChange = function findLatestTimeChange (sbx) {
    ...
    if (cannulaInfo.age >= prefs.urgent) {
      ...
      message = translate('Change cannula soon');
    ...
    if (prefs.display === 'days' && cannulaInfo.found) {
      ...
      if (cannulaInfo.age >= 24) {
        cannulaInfo.display += cannulaInfo.days + ' nap '; // HU translation
      }
      cannulaInfo.display += cannulaInfo.hours + ' óra'; // HU translation
    } else {
      cannulaInfo.display = cannulaInfo.found ? cannulaInfo.age + ' óra' : 'n/a ';
    }
    ...
    if (prefs.enableAlerts && sendNotification && cannulaInfo.minFractions <= 20) {
      cannulaInfo.notification = {
        title: 'Előző pumpa csere '+ cannulaInfo.display +' ezelőtt' // translate('Cannula age %1 hours', { params: [cannulaInfo.age] }) // replacing message with one formatted, translated above
        , message: message
        , pushoverSound: sound
