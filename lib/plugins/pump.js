...
function init (ctx) {
  ...
  function updateClock (prefs, result, sbx) {
    if (result.clock) {
      ...
      if (urgent.isAfter(result.clock.value)) {
        ...
        result.clock.message = '⚠️ Csatlakoztasd a kütyüt! 📟';
      } else if (warn.isAfter(result.clock.value)) {
        result.clock.level = levels.WARN;
        result.clock.message = 'Csatlakoztasd a kütyüt! 📟';
      } else {
        result.clock.level = levels.NONE;
      }
    } else {
      // pump data so old, not available anymore
      result.clock = { }; 
      result.clock.display = 'NA'; // appropriate?
      result.clock.label = 'Clock NA';
      result.clock.level = levels.URGENT;
      result.clock.message = '⚠️ Csatlakoztasd a kütyüt! 📟';
    }
  }
  ...
  function updateReservoir (prefs, result) {
    if (result.reservoir) {
      ...
      if (result.reservoir.value < prefs.urgentRes) {
        ...
        result.reservoir.message = '⚠️ Kevés az inzulin! ⚗️';
      } else if (result.reservoir.value < prefs.warnRes) {
        result.reservoir.level = levels.WARN;
        result.reservoir.message = 'Kevés az inzulin! ⚗️';
      } else {
  ...
  function updateBattery (type, prefs, result) {
    if (result.battery) {
      ...
      if (result.battery.value < urgent) {
        ...
        result.battery.message = '⚠️ Cseréld ki a pumpa elemet! 🔋';
      } else if (result.battery.value < warn) {
        result.battery.level = levels.WARN;
        result.battery.message = 'Cseréld ki a pumpa elemet! 🔋';
      } else {
  ...
  function buildMessage (result) {
    if (result.level > levels.NONE) {
      ...
      if (result.clock) {
        if (result.clock.display == 'NA') { // pump data so old, not available anymore    
          message.push('Nincs pumpa adat'); // HU 'No pump data at all
        } else {
          message.push('Utolsó pumpa adat ' + result.clock.display); // HU
        }   
      }

      if (result.battery) {
        message.push('Pumpa 🔋: ' + result.battery.display);
      }

      if (result.reservoir) {
        message.push('Pumpa tartály: ' + result.reservoir.display);
      }
