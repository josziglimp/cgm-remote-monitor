...
function init(ctx) {
  ...
  timeago.checkStatus = function checkStatus(sbx) {
    ...
    // Timeago seems to be continually suspended, hardly ever getting the expected alarm
  ...
  timeago.resolvers = [
    timeago.isMissing, timeago.inTheFuture, timeago.almostInTheFuture, timeago.isLessThan(times.mins(2).msecs, times.min().msecs, 'perce', 'm'), timeago.isLessThan(times.hour().msecs, times.min().msecs, 'perce', 'm'), timeago.isLessThan(times.hours(2).msecs, times.hour().msecs, 'órája', 'h'), timeago.isLessThan(times.day().msecs, times.hour().msecs, 'órája', 'h'), timeago.isLessThan(times.days(2).msecs, times.day().msecs, 'napja', 'd'), timeago.isLessThan(times.week().msecs, times.day().msecs, 'napja', 'd'), // translated to HU
    function () {
      return {
        label: 'long ago',
        shortLabel: 'ago'
